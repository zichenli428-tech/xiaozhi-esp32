...
def _idf_command(*args: str) -> list[str]:
    """Build the idf.py command line.

    On Windows subprocess.run() cannot launch a bare .py script found on PATH
    (CreateProcess fails with WinError 193), so invoke the interpreter explicitly
    when the IDF source tree is known.
    """
    idf_py = Path(os.environ.get("IDF_PATH", "")) / "tools" / "idf.py"
    if idf_py.is_file():
